...
class SpecialItems extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GetX<HomeGetxController>(
        builder: (controller){
                return controller.isLoading.value ? const ShimmerCardHomeLoading() : controller.paidItems.isEmpty ?const Center(
                          child: Text('لا توجد اي عناصر '),
                        ) : ListView.separated(
                  padding: EdgeInsets.zero,
                  shrinkWrap: true,
                  ...
                  const NeverScrollableScrollPhysics(),
                  // scrollDirection: Axis.horizontal,
                  itemCount:controller.configs.first.data!.itemNumberPaid != null?
                  int.parse(controller.configs.first.data!.itemNumberPaid ?? "")
                  : controller.paidItems.length,
                  separatorBuilder: (context, index) =>
                      16.ESH(),
                  itemBuilder: (context, index) {
                    return HomeWidget(
                      ...
                      onTap: () {
                        Get.to(
                              () => DetailsScreen(
                            id: controller.paidItems[index].id.toString(),
                          ),
                        );
                      },
                      saveOnPressed: () => saveItem(
                          id: controller.paidItems[index].id
                              .toString()),
                      discount: '25',
                      image: controller.paidItems[index].itemImage,
                      itemType: controller.paidItems[index].itemCategoriesString,
                      location:
                    controller.paidItems[index].itemDescription ?? "",
                      title: controller.paidItems[index].itemTitle,
                      rate: controller.paidItems[index].itemAverageRating,
                    );
                  },
                );
    });
