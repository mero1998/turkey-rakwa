...
class AllNearsetGetxController extends GetxController  with StateMixin<List<PaidItemsModel>>, ScrollMixin{
  static AllNearsetGetxController get to => Get.find();
  ...
  void onInit() {
    super.onInit();
    // getNestedItemsPagenation(current_page: page, type: 0);
  }


  Future<void> getNestedItemsPagenation({required int current_page,required int type}) async {
   print("we are here from read");
   if(current_page == 1){
     nestedItemsMore.clear();
     isLoading.value = true;
   }
    List<PaidItemsModel> items =   await HomeApiController().getNearestItemsWithPagenation(current_page:current_page , type:type);
  ...
  Future<void> onEndScroll() async{
    ...
    if (!lastPage) {
      ...
      await getNestedItemsPagenation(current_page: page, type: 0);
      // Get.back();
      loading.value = false;
