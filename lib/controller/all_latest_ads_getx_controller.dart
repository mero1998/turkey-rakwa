...
class AllLatestAdsGetxController extends GetxController  with StateMixin<List<PaidItemsModel>>, ScrollMixin{
  static AllLatestAdsGetxController get to => Get.find();
  RxBool isLoading = true.obs;

  RxList<PaidItemsModel> latestAdsMore = <PaidItemsModel>[].obs;
  ...
  void onInit() {
    super.onInit();
    getLatestAdsPagenation(current_page: page);
  }


  Future<void> getLatestAdsPagenation({required int current_page}) async {
    print("we are here");
    List<PaidItemsModel> items =   await HomeApiController().getLatestClassifiedWithPagenation(current_page:current_page );
  ...
  Future<void> onEndScroll() async{
    ...
    if (!lastPage) {
      ...
      await getLatestAdsPagenation(current_page: page);
      // Get.back();
      loading.value = false;
