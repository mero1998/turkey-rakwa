...
class AllPopularClassifiedGetxController extends GetxController  with StateMixin<List<PaidItemsModel>>, ScrollMixin{
  static AllPopularClassifiedGetxController get to => Get.find();
  RxBool isLoading = true.obs;

  RxList<PaidItemsModel> popularClassifiedMore = <PaidItemsModel>[].obs;
  ...
  void onInit() {
    super.onInit();
    getPopularClassifiedPagenation(current_page: page);
  }


  Future<void> getPopularClassifiedPagenation({required int current_page}) async {
    print("we are here");
    List<PaidItemsModel> items =   await HomeApiController().getPopularClassifiedWithPagenation(current_page:current_page );
    ...
    if (!getFirstData && emptyRepositories) {
      ...
      popularClassifiedMore.addAll(items);
  ...
  Future<void> onEndScroll() async{
    ...
    if (!lastPage) {
      ...
      await getPopularClassifiedPagenation(current_page: page);
      // Get.back();
      loading.value = false;
