...
class AllNearAdsGetxController extends GetxController  with StateMixin<List<PaidItemsModel>>, ScrollMixin{
  static AllNearAdsGetxController get to => Get.find();
  RxBool isLoading = true.obs;

  RxList<PaidItemsModel> nearAdsMore = <PaidItemsModel>[].obs;
  ...
  void onInit() {
    super.onInit();
    getNearAdsPagenation(current_page: page);
  }


  Future<void> getNearAdsPagenation({required int current_page}) async {
    print("we are here");
    List<PaidItemsModel> items =   await HomeApiController().getNearestClassifiedWithPagenation(type: 1,current_page:current_page );
  ...
  Future<void> onEndScroll() async{
    ...
    if (!lastPage) {
      ...
      await getNearAdsPagenation(current_page: page);
      // Get.back();
      loading.value = false;
