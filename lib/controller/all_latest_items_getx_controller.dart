...
class AllLatestItemsGetxController extends GetxController  with StateMixin<List<PaidItemsModel>>, ScrollMixin{
  static AllLatestItemsGetxController get to => Get.find();
  RxBool isLoading = true.obs;

  RxList<PaidItemsModel> latestItemsMore = <PaidItemsModel>[].obs;
  ...
  void onInit() {
    super.onInit();
    getLatestItemsPagenation(current_page: page);
  }


  Future<void> getLatestItemsPagenation({required int current_page}) async {
    print("we are here");
    List<PaidItemsModel> items =   await HomeApiController().getLatestItemsWithPagenation(current_page:current_page );
  ...
  Future<void> onEndScroll() async{
    ...
    if (!lastPage) {
      ...
      await getLatestItemsPagenation(current_page: page);
      // Get.back();
      loading.value = false;
