...
class AllOrdersVendorGetxController extends GetxController with StateMixin<List<OrdersVendor>>,ScrollMixin{
  static AllOrdersVendorGetxController get to => Get.find();
  ...
  // List<int> vendorsIds = <int>[].obs;
  // RxList<Orders> orders = <Orders>[].obs;
  RxList<OrdersVendor> ordersVendor = <OrdersVendor>[].obs;
  @override
  void onInit() {
    super.onInit();
    // getOrders(current_page: page);
    getOrdersVendor(current_page: page);
  ...
  Future<void> onEndScroll() async{
    ...
    if (!lastPage) {
      ...
      await getOrdersVendor(current_page: page);
      // Get.back();
      loading.value = false;
...
// DetailsApiController().getDetails(id: widget.id.toString())
  ...
  Future<void> getOrdersVendor({required int current_page}) async {
    print("we are here");
    List<OrdersVendor> order =   await OrderApiController().getOrdersVendor(current_page:current_page );
