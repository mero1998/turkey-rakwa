...
class AllPopularItemsGetxController extends GetxController  with StateMixin<List<PaidItemsModel>>, ScrollMixin{
  static AllPopularItemsGetxController get to => Get.find();
  RxBool isLoading = true.obs;

  RxList<PaidItemsModel> popularItemsMore = <PaidItemsModel>[].obs;
  ...
  void onInit() {
    super.onInit();
    getPopularItemsPagenation(current_page: page);
  }


  Future<void> getPopularItemsPagenation({required int current_page}) async {
    List<PaidItemsModel> items =   await HomeApiController().getPopularItemsWithPagenation(current_page:current_page );
  ...
  Future<void> onEndScroll() async{
    ...
    if (!lastPage) {
      ...
      await getPopularItemsPagenation(current_page: page);
      // Get.back();
      loading.value = false;
