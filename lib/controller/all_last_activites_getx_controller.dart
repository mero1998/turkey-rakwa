...
import '../api/api_controllers/activites_api_controller.dart';
...
class AllLastActivtiesGetxController extends GetxController  with StateMixin<List<LastActivites>>, ScrollMixin{
  static AllLastActivtiesGetxController get to => Get.find();
  RxBool isLoading = true.obs;

  RxList<LastActivites> allActivitesMore = <LastActivites>[].obs;
  ...
  void onInit() {
    super.onInit();
    getActivitiesPagenation(current_page: page);
  }


  Future<void> getActivitiesPagenation({required int current_page}) async {
    print("we are here");
    List<LastActivites> items =   await ActivitiesApiController().getActivitiesWithPagnation(current_page:current_page );
  ...
  Future<void> onEndScroll() async{
    ...
    if (!lastPage) {
      ...
      await getActivitiesPagenation(current_page: page);
      // Get.back();
      loading.value = false;
